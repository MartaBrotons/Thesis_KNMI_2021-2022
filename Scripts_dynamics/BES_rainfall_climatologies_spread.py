# This script generates rainfall climatologies on the region of the caribbean from observations (GPCP) and CMIP6 multimodel mean and 90% spread. (Figure 5 report)
...
plt.plot(cmip6_mean.month, cmip6_mean.pr*86400, label='CMIP6', color = 'steelblue',linewidth=3)
plt.fill_between(cmip6_month.month,cmip6_month.pr.quantile(0.05,dim='model')*86400,cmip6_month.pr.quantile(0.95,dim='model')*86400,color='lightskyblue',linewidth=0)
cmip6_90 =  mpatches.Patch( color='lightskyblue', label=r'$90\%$ CMIP6 ')
mean = mlines.Line2D([], [], color='steelblue', label='CMIP6 multimodel mean')
gpcp = mlines.Line2D([], [], color='black', label='GPCP observations')
plt.legend(handles=[gpcp,mean,cmip6_90],fontsize = 18,loc = 'upper left')
#plt.plot(gpcp.month,gpcp.precip, label='GPCP (1979-2019)')
...
ax.vlines(4.5,-1,8, color = 'gray',linestyle = '--', linewidth = 1)
ax.vlines(11.5,-1,8, color = 'gray',linestyle = '--', linewidth = 1)
ax.text(1.5,-.2,'Dry season',fontsize = 18,color = 'gray')
ax.text(7,-.2,'Wet season',fontsize = 18,color = 'gray')
ax.set_xticks(gpcp_mean.month)
ax.set_xticklabels(months,rotation=50,fontsize = 18)
ax.set_yticklabels(np.linspace(-1,7,9),fontsize = 18)
ax.set_ylim(-0.5,7.8)
#plt.xlabel("Month of the year")
plt.ylabel("Precipitation [mm/day]",fontsize = 18)
#plt.title("Caribbean precipitation climatologies (1991-2020)",fontsize = 20)
plt.savefig('/home/bbrotonsbl/shared_data/volume_2/bbrotonsbl/resampling/Method-2014/output_plots/pwpt/BES_rainfall_climatologies_2seasons.png')