# Python Script to compute 30 years trends for CMIP6 models for the reference period (approx). (Figure 29)
...
if var == 'Niño34':
    ...
    ts = ts.load()
    ts = ts.ts
    ts = ts.sel(time=slice('1-1-1979', '31-12-2020'),drop = True)
    ...
    # ERA5 data:
    #ts_era5_nino34 = xr.open_dataset(paths_era5['sst'])
    ts_era5 = xr.open_dataset(paths_era5['sst'])
    ts_era5_nino34 = ts_era5.where((ts_era5.latitude > latitude[0]) & (ts_era5.latitude < latitude[1]) & (ts_era5.longitude > longitude[0]) & (ts_era5.longitude < longitude[1]), drop=True)
    ts_era5_nino34 = ts_era5_nino34.where(ts_era5_nino34.expver == 1,drop = True)
    ts_era5_nino34_mean = boxmean_era(ts_era5_nino34.sst)
    ts_era5_nino34 = ts_era5_nino34_mean.sel(time=slice('1-1-1979', '31-12-2020'),drop = True)
    ts_era5_nino34_ref = ts_era5_nino34_mean.sel(time=slice('01-01-1991', '31-12-2020')).mean(dim = 'time')
    ds_era5 = ts_era5_nino34 -ts_era5_nino34_ref
    ...
    tas_NASPG = tas_NASPG.sel(time=slice('1-1-1979', '31-12-2020'),drop = True)
    ...
    # ERA5 data:
    tas_era5 = xr.open_dataset(paths_era5['tas'])
    breakpoint()
    tas_NASPG_era5 = tas_era5.where((tas_era5.latitude > 50 ) & (tas_era5.latitude < 60) & (tas_era5.longitude > 305) & (tas_era5.longitude < 340), drop=True)
    tas_NASPG_era5 = tas_NASPG_era5.where(tas_NASPG_era5.expver == 1,drop = True)
    tas_NASPG_era5_mean = boxmean_era(tas_NASPG_era5.t2m)
    tas_NASPG_era5 = tas_NASPG_era5_mean.sel(time=slice('1-1-1979', '31-12-2020'),drop = True)
    tas_NASPG_era5_ref = tas_NASPG_era5_mean.sel(time=slice('1-1-1991', '31-12-2020'),drop = True).mean(dim = 'time')
    ds_era5 = tas_NASPG_era5 - tas_NASPG_era5_ref
...
if scaling == True:
    ...
    #tas_40N_40S_reg = tas_40N_40S_mean.sel(time=slice('1-1-2059', '31-12-2100'),drop = True)
    tas_40N_40S = tas_40N_40S.sel(time=slice('1-1-1979', '31-12-2020'),drop = True)
    tas_40N_40S_ref = tas_40N_40S.sel(time=slice('1-1-1991', '31-12-2020'),drop = True).mean(dim = 'time')
    tas_40N_40S['time'] = ds.time 
    tas_40N_40S = tas_40N_40S.sortby(tas_40N_40S.model_name)
    ds = ds/(tas_40N_40S-tas_40N_40S_ref)

    # Era5
    tas_40N_40S_era5 = xr.open_dataset(paths_era5['tas'])
    tas_40N_40S_era5 = tas_40N_40S_era5.where((tas_40N_40S_era5.latitude > -40) & (tas_40N_40S_era5.latitude < 40), drop=True)
    tas_40N_40S_era5 = boxmean_era(tas_40N_40S_era5.t2m) 
    tas_40N_40S_era5 = tas_40N_40S_era5.where(tas_40N_40S_era5.expver == 1,drop = True)
    tas_40N_40S_era5 = tas_40N_40S_era5.sel(time=slice('1-1-1979', '31-12-2020'),drop = True)
    tas_40N_40S_era5_ref = tas_40N_40S_era5.sel(time=slice('1-1-1991', '31-12-2020'),drop = True).mean(dim = 'time')
    ds_era5 = ds_era5/(tas_40N_40S_era5 - tas_40N_40S_era5_ref)
    ...
    # # BES2 30 and 70
    # wet_models = ['CIESM','CNRM-CM6-1','CNRM-CM6-1-HR','CNRM-ESM2-1','EC-Earth3','FGOALS-f3-L','INM-CM4-8','INM-CM5-0','MIROC6','NESM3']
    # dry_models = ['ACCESS-CM2','CESM2','CESM2-WACCM','CanESM5','CanESM5-CanOE','HadGEM3-GC31-LL','IPSL-CM6A-LR','MPI-ESM1-2-LR','MRI-ESM2-0','UKESM1-0-LL']
...
for season in season_list:
    ...
    ds_era5_period = time_period(ds_era5,season)
    ...
    #ERA5 reanalysis
    ds_era5_rolled = ds_era5_period.rolling(time=5,center=False).mean(dim='time').dropna(dim='time')
    ds_era5_yearly = ds_era5_rolled.groupby(ds_era5_rolled.time.dt.year).mean(dim = 'time').dropna(dim='year')
    ...
    for i in range(0,11): #years
        ...
        m_era5.append(np.polyfit(ds_era5_yearly.year[i:i+30],ds_era5_yearly[i:i+30], 1)[0])
    ...
    ax.set_title(title_ssp[ssp],loc='left',fontsize=12)
    ax.set_title(season_title[season],loc='right',fontsize=12)
    ax.scatter(m_cmip6[0,:],years, s = 5,color = 'blue',label = 'CMIP6 models')
    ...
    ax.scatter(m_era5,years, s = 85,marker = 'X',color = 'orange',label = 'ERA5 reanalysis data')
    ax.set_xlabel(clabel_trend[var],fontsize=12)
    ax.set_ylabel('Year',fontsize=12)
    ax.legend(loc = 'lower right',fontsize=12)
    ax.tick_params(axis='both', which='major', labelsize=12)
    #fig.savefig('output_plots/'+var+sc+'_trend_future_CMIP6_observations.png')
    fig.savefig('/home/bbrotonsbl/shared_data/volume_2/bbrotonsbl/resampling/Method-2014/output_plots/report/'+var+sc+'_trend_CMIP6_observations_'+season+'_season.png',dpi = 1200)
